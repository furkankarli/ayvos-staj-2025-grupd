...
image_path = 'Segmentasyon/photo.jpg'  # Deneme görseli
...
instance_masks_yolo = yolo_result.masks.data.cpu().numpy() if yolo_result.masks is not None else []
...
semantic_yolo = np.sum(instance_masks_yolo, axis=0) if len(instance_masks_yolo) > 0 else np.zeros((640, 640))
semantic_rcnn = np.sum(instance_masks_rcnn, axis=0) if len(instance_masks_rcnn) > 0 else np.zeros((640, 640))
...
axs[1, 0].imshow(np.ma.masked_where(semantic_yolo < 0.5, semantic_yolo), cmap='Greens', alpha=0.5)

axs[1, 1].imshow(image_rgb)
axs[1, 1].set_title('Mask R-CNN - Semantic Segmentation (benzeri)')
axs[1, 1].imshow(np.ma.masked_where(semantic_rcnn < 0.5, semantic_rcnn), cmap='Reds', alpha=0.5)
