...
def draw_border(
    ...
    """
    Draws stylized corner borders around a bounding box.
    """
    x1, y1 = top_left
    x2, y2 = bottom_right
...
# ----------------------------
# Load interpolated detection results
# ----------------------------
...
# ----------------------------
# Extract the best license plate prediction for each car
# ----------------------------
license_plate = {}
for car_id in np.unique(results["car_id"]):
    ...
    # Seek to the frame with the best license plate detection
    cap.set(
...
# ----------------------------
# Draw bounding boxes and license plates on video
# ----------------------------
frame_idx = -1
...
while ret:
    ...
    frame_idx += 1
    if ret:
        df_ = results[results["frame_idx"] == frame_idx]
        ...
        for row_idx in range(len(df_)):
            ...
            # ----------------------------
            x1, y1, x2, y2 = ast.literal_eval(
                df_.iloc[row_idx]["license_plate_bbox"]
                .replace("[ ", "[")
                .replace("   ", " ")
                .replace("  ", " ")
                .replace(" ", ",")
            ...
            # ----------------------------
            # Overlay cropped license plate & plate number text
            # ----------------------------
            license_crop = license_plate[df_.iloc[row_idx]["car_id"]]["license_crop"]
            H, W, _ = license_crop.shape

            try:
                # Place cropped license plate above the car
                frame[
                ...
                (text_width, text_height), _ = cv2.getTextSize(
                    license_plate[df_.iloc[row_idx]["car_id"]]["license_plate_number"],
