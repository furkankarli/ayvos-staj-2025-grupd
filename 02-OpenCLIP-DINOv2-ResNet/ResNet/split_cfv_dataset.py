...
def generate_split_json(data_dir="dataset", label_json="data.json", output_dir="."):
    ...
    with open(os.path.join(output_dir, "../data/train.json"), "w") as f:
        json.dump(train_list, f, indent=2)
    with open(os.path.join(output_dir, "../data/val.json"), "w") as f:
        json.dump(val_list, f, indent=2)
    with open(os.path.join(output_dir, "../data/test.json"), "w") as f:
...
if __name__ == "__main__":
    generate_split_json(
        data_dir="dataset", label_json="data.json", output_dir="../data"
    )
